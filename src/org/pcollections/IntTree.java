 ...
 * decreasing all keys below a certain value, the keys values are stored
 * relative to their parent. This makes this map a good backing for fast
 * insertion and removal of indices in a vector.
 * <p>
 ...
 * Data.Map implementation, which in turn is based on
 * "size balanced binary trees" as described by:
 * <p>
...
class IntTree<V> {
	...
	// there might be some way to deal with this based on left-verse-right
	// logic,
	...
	private IntTree(final long key, final V value, final IntTree<V> left,
			final IntTree<V> right) {
		this.key = key;
		this.value = value;
		this.left = left;
		this.right = right;
	...
	IntTree<V> plus(final long key, final V value) {
		...
		// otherwise key==this.key, so we simply replace this, with no effect on
		// balance:
		if (value == this.value) {
			return this;
		}
	...
	IntTree<V> minus(final long key) {
		...
		// otherwise replace this with the next key (i.e. the smallest key to
		// the right):

		// TODO have minNode() instead of minKey to avoid having to call get()
		// TODO get node from larger subtree, i.e. if left.size>right.size use
		// left.maxNode()
		// TODO have faster minusMin() instead of just using minus()

		long newKey = right.minKey() + this.key;
		// (right.minKey() is relative to this; adding this.key makes it
		// 'absolute'
		...
		// lastly, make the subtree keys relative to newKey (currently they are
	 ...
	 * 
	 * This method will create an _invalid_ tree if delta<0 and the distance
	 * between the smallest k>=key in this and the largest j<key in this is
	 * |delta| or less.
	 * 
	 * In other words, this method must not result in any change in the order of
	 * the keys in this, since the tree structure is not being changed at all.
	 */
	IntTree<V> changeKeysAbove(final long key, final int delta) {
		...
		if (this.key >= key) {
			// adding delta to this.key changes the keys of _all_ children of
			// this,
			// so we now need to un-change the children of this smaller than
			// key,
			// all of which are to the left. note that we still use the 'old'
			// relative key...:
			return new IntTree<>(this.key + delta, value, left.changeKeysBelow(
					key - this.key, -delta), right);
		}
	 ...
	 * 
	 * This method will create an _invalid_ tree if delta>0 and the distance
	 * between the largest k<key in this and the smallest j>=key in this is
	 * delta or less.
	 * 
	 * In other words, this method must not result in any overlap or change in
	 * the order of the keys in this, since the tree _structure_ is not being
	 * changed at all.
	 */
	IntTree<V> changeKeysBelow(final long key, final int delta) {
		...
		if (this.key < key) {
			...
			// all of which are to the right. note that we still use the 'old'
			// relative key...:
			return new IntTree<>(this.key + delta, value, left,
					right.changeKeysAbove(key - this.key, -delta));
		}
	...
	private IntTree<V> rebalanced(final IntTree<V> newLeft,
			final IntTree<V> newRight) {
	...
	private static <V> IntTree<V> rebalanced(final long key, final V value,
		...
		if (left.size + right.size > 1) {
			if (left.size >= OMEGA * right.size) { // rotate to the right
				...
				if (lr.size < ALPHA * ll.size) {
					return new IntTree<>(left.key + key, left.value, ll,
							new IntTree<>(-left.key, value, lr.withKey(lr.key
									+ left.key), right));
				} else { // double rotation:
					IntTree<V> lrl = lr.left, lrr = lr.right;
					return new IntTree<>(lr.key + left.key + key, lr.value,
							new IntTree<>(-lr.key, left.value, ll, lrl
									.withKey(lrl.key + lr.key)),
							new IntTree<>(-left.key - lr.key, value, lrr
									.withKey(lrr.key + lr.key + left.key),
									right));
				}
			} else if (right.size >= OMEGA * left.size) { // rotate to the left
				IntTree<V> rl = right.left, rr = right.right;
				if (rl.size < ALPHA * rr.size) {
					return new IntTree<>(right.key + key, right.value,
							new IntTree<>(-right.key, value, left,
									rl.withKey(rl.key + right.key)), rr);
				} else { // double rotation:
					IntTree<V> rll = rl.left, rlr = rl.right;
					return new IntTree<>(rl.key + right.key + key, rl.value,
							new IntTree<>(-right.key - rl.key, value, left, rll
									.withKey(rll.key + rl.key + right.key)),
							new IntTree<>(-rl.key, right.value, rlr
									.withKey(rlr.key + rl.key), rr));
				}
			}
		}
	...
	private static final class EntryIterator<V> implements
			Iterator<Entry<Integer, V>> {
		private PStack<IntTree<V>> stack = ConsPStack.empty(); // path of
																// nonempty
																// nodes
		private int key = 0; // note we use _int_ here since this is a truly
								// absolute key
		...
		public Entry<Integer, V> next() {
			...
			final Entry<Integer, V> result = new org.pcollections.SimpleImmutableEntry<>(
					key, node.value);
			...
			if (node.right.size > 0) {
				...
				while (true) { // find current node's least larger ancestor, if
								// any
					key -= node.key; // revert to parent's key
					stack = stack.subList(1); // climb up to parent
					// if parent was larger than child or there was no parent,
					// we're done:
