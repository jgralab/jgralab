...
public class GraphIO {
	...
	private GraphBaseImpl graph(ProgressFunction pf,
		...
		try {
			graph = (GraphBaseImpl) this.schema.getGraphCreateMethod(
	...
	throws GraphIOException {
		...
		try {
			...
			this.vertexDescTempObject[0] = vId;
			vertex = (Vertex) createMethod.invoke(graph, this.vertexDescTempObject);
		} catch (Exception e) {
			e.printStackTrace();
	...
	throws GraphIOException {
		...
		try {
			...
			this.edgeDescTempObject[0] = eId;
			this.edgeDescTempObject[1] = this.edgeOut[eId];
			this.edgeDescTempObject[2] = this.edgeIn[eId];
			edge = (Edge) createMethod.invoke(graph, this.edgeDescTempObject);
		} catch (Exception e) {
