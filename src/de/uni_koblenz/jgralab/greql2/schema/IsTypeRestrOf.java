/*
 * This code was generated automatically.
 * Do NOT edit this file, changes will be lost.
 * Instead, change and commit the underlying schema.
 */

package de.uni_koblenz.jgralab.greql2.schema;

import de.uni_koblenz.jgralab.Aggregation;
import de.uni_koblenz.jgralab.EdgeDirection;

import de.uni_koblenz.jgralab.greql2.schema.impl.IsTypeRestrOfImpl;
/**
FromVertexClass: TypeId
FromRoleName : 
ToVertexClass: Expression
toRoleName : 
 */

public interface IsTypeRestrOf extends Aggregation, Greql2Aggregation {

	/**
	 * refers to the default implementation class of this interface
	 */
	public static final Class<IsTypeRestrOfImpl> IMPLEMENTATION_CLASS = IsTypeRestrOfImpl.class;

	/**
	 * @return the next Greql2Aggregation edge in the global edge sequence
	 */
	public Greql2Aggregation getNextGreql2AggregationInGraph();

	/**
	 * @return the next IsTypeRestrOf edge in the global edge sequence
	 */
	public IsTypeRestrOf getNextIsTypeRestrOfInGraph();

	/**
	 * @return the next IsTypeRestrOf edge in the global edge sequence
	 * @param noSubClasses if set to <code>true</code>, no subclasses of IsTypeRestrOf are accepted
	 */
	public IsTypeRestrOf getNextIsTypeRestrOfInGraph(boolean noSubClasses);

	/**
	 * @return the next edge of class Greql2Aggregation at the "this" vertex
	 */
	public Greql2Aggregation getNextGreql2Aggregation();

	/**
	 * @return the next edge of class Greql2Aggregation at the "this" vertex
	 * @param orientation the orientation of the edge
	 */
	public Greql2Aggregation getNextGreql2Aggregation(EdgeDirection orientation);

	/**
	 * @return the next edge of class IsTypeRestrOf at the "this" vertex
	 */
	public IsTypeRestrOf getNextIsTypeRestrOf();

	/**
	 * @return the next edge of class IsTypeRestrOf at the "this" vertex
	 * @param orientation the orientation of the edge
	 */
	public IsTypeRestrOf getNextIsTypeRestrOf(EdgeDirection orientation);

	/**
	 * @return the next edge of class IsTypeRestrOf at the "this" vertex
	 * @param noSubClasses if set to <code>true</code>, no subclasses of IsTypeRestrOf are accepted
	 */
	public IsTypeRestrOf getNextIsTypeRestrOf(boolean noSubClasses);

	/**
	 * @return the next edge of class IsTypeRestrOf at the "this" vertex
	 * @param orientation the orientation of the edge
	 * @param noSubClasses if set to <code>true</code>, no subclasses of IsTypeRestrOf are accepted
	 */
	public IsTypeRestrOf getNextIsTypeRestrOf(EdgeDirection orientation, boolean noSubClasses);

}
