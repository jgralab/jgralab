...
public class Greql2FunctionLibrary {
	...
	static {
		...
		if (DEBUG)
			System.out.println("Nondotted Package name " + nondottedPackageName);
		thisInstance = new Greql2FunctionLibrary();
		if (DEBUG)
			System.out.println("FunctionLibrary successfull loaded");
	}
	...
	public Greql2FunctionLibrary() {
		//System.out.println("Creating a new GreqlFunctionLibrary");
	...
	/**
	 *  transforms the given className to a valid GReQL Functionname. 
	 *  This means, the first letter of the className is transformedd to lowercase.
	 *  So the className <code>IsSupertype</code> will become the 
	 *  functionName <code>isSupertype</code> 
	 */
	...
	private void registerFunction(String className) { 
		try {
			if (DEBUG) {
				System.out.println("Try to register function: " + className);
				System.out.println("Found Class: " + (Class.forName(packageName + "." + className) != null));
			}
			Class[] interfaces = Class.forName(packageName + "." + className).getInterfaces();
			String funIntName = packageName + ".Greql2Function";
			for (int i = 0; i<interfaces.length; i++) {
				if (DEBUG)
					System.out.println("Implementing interface " + interfaces[i].getName() );
				if (interfaces[i].getName().equals(funIntName)) {
					Object o = Class.forName(packageName + "." + className).getConstructor().newInstance();
					availableFunctions.put(toFunctionName(className), (Greql2Function) o );
				}	
			}
		} 
		catch (Exception ce) {}
	...
	private boolean registerFunctionsInJar(String packagePath) {
		if (packagePath.lastIndexOf(".jar!/") > 0) {
			...
			System.out.println("Path of package is: " + packagePath);
			try {
				JarFile jar = new JarFile(packagePath);
				System.out.println("Try to read entrys");
				for (Enumeration<JarEntry> e = jar.entries(); e.hasMoreElements();  ) {
					JarEntry je = e.nextElement();
					String entryName = je.getName();
					if (DEBUG && entryName.contains("funlib"))
							System.out.println("Reading entry " + entryName);
					if (entryName.startsWith(nondottedPackageName)	&& entryName.endsWith(".class")) {
						registerFunction(entryName.substring(nondottedPackageName.length()+1, entryName.length() - 6));
						if (DEBUG)
							System.out.println("Registering function: " + entryName.substring(nondottedPackageName.length()+1, entryName.length() - 6));
					}
				}
			} catch (Exception e) {
			}
	...
	private boolean registerFunctionsInDirectory(String packagePath) {
		...
		if (DEBUG)
			System.out.println("Directory Path : " + packagePath);
		String entries[] = new File(packagePath).list();
		int i = 0;
		for (i = 0; i < entries.length; i++) {
			//System.out.println("Entriename " + entries[i]);
			if (entries[i].endsWith(".class")) {
				String className = entries[i].substring(0, entries[i].length()-6);
				//System.out.println("Try to register function : " + className);
	...
	private boolean registerAllFunctions() {
		if (DEBUG)
			System.out.println("Registering all functions");
		availableFunctions = new HashMap<String, Greql2Function>();
		String thisClassName = this.getClass().getCanonicalName();
		if (DEBUG)
			System.out.println("Functionlib name: " + thisClassName);
		URL packageUrl = Greql2FunctionLibrary.class.getResource("/" + nondottedPackageName + "/Greql2FunctionLibrary.class");
		if (packageUrl != null) {
			if (DEBUG) {
				System.out.println("Found Greql2FunctionLibrary");
				System.out.println("URL : " + packageUrl.getPath());
			}	
			...
			if (!packagePath.startsWith("/")) {
				...
				//System.out.println("Stripped path : " + packagePath);
			}
