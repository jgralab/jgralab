...
/**
 * A JValueTable is the Java "replacement" for CvTable (but in fact it has not much common with it)
 * It's based on bag of Tuples with an additional header tuple. Theoretical, it's possible to store any 
 * kind of objects in the bag, but to hold the implementation and usage clean and fast, ony tuple are allowed
...
public class JValueTable extends JValueCollection {
	...
	/**
	 * This is the internal datastructure. Normaly, this is a bag, but the implementation of JValueTable
	 * is independent of the real object, so also list, set oder some other collection can be used
	 */
	...
	/**
	 * returns this table as set, that is the databag without the header tuple, duplicates will be eliminated
	 */
	...
	/**
	 * returns this table as tuple, that is the databag as tuple without the header tuple
	 */
	...
	/**
	 * returns this table as list, that is the databag as list without the header tuple
	 */
	...
	/**
	 * returns this table as record, the header is accessible as record.tableHeader while all
	 * other elements are accessible as record.1, record.2 etc
	 */
	...
	/**
	 * creates a new JValueTable with an empty header
	 * @param useSet if it is true, the table will be based on a <b>set</b> instead of a bag, so 
	 * every element can exists only one in the table 
	 */
	...
	/**
	 * creates a new JValueTable which is based on a bag and sets the given tuple as header of that table
	 */
	...
	/**
	 * creates a new JValueTable and sets the given tuple as header of that table 
	 * @param useSet if it is true, the table will be based on a <b>set</b> instead of a bag, so 
	 * every element can exists only one in the table 
	 */
	...
	/**
	 * creates a new JValueTable which contains each element of the given collection
	 * as seperate row. The table doesn't have a header
	 */
	...
	public void printTable() {
		...
		while (headIter.hasNext()) {
			System.out.print("|");
			System.out.print(headIter.next());
		}
		System.out.println();
		System.out.println("----------------------------------------------------------------------");
		System.out.println("----------------------------------------------------------------------");
	...
	/**
	 * adds the given Element to this Table. If the element is not a tuple, it will be encapsulated in a JValue.
	 * Because for JValueBag and JValueSet the elementorder is not fix, the given JValue is, even if it is a collection, not
	 * transformed into a tuple, instead of this its used as the first element of a new created tuple
	 */
	...
	/**
	 * removes all elements from the datastructure, the header doesn't get cleaned
	 */
	public void clear() {
		data.clear();
	}
	...
	/**
	 * @return the number of elements in the datastructure, that is the number of tablerows
	 */
	public int size() {
		return data.size();
	}
	...
	/**
	 * @return true if the datastructure contains
	 *   - the given tuple 
	 * or
	 *   - a tuple which contains the same elements in the same order
	 * or
	 *   - a tuple which contains the given element as first element  
	 */
				...
				if (col.isJValueTuple()) {
					...
					while (iter.hasNext()) {
						...
						if (curTup.size() == tup.size()) {
							int i = 0;
							while ((tup.get(i).equals(curTup.get(i))) && (i < tup.size()) ) {
								i++;
							}
							if (i == tup.size());
								return true;
	...
	/**
	 * Returns <code>true</code> if at least one value of the specified column appears twice.
	 * Otherwise, if every value of the column appears only once or if the specified column 
	 * don't exist, <code>false</code> is returned.
	 * @param colNumber The column to check for duplicated entries. Index of first column is 1.
	 */
	public boolean columnContainsDuplicateEntries(int colNumber){
		if (this.size() < 1 || colNumber < 1) return false;
		...
		while(tupleIter.hasNext()){
			...
			if (colNumber > tuple.size()) return false; //not nice!! ckeck should be part of first if of method. need a getColumnCount() method? 
	...
	/**
	 * removes the given JValueTuple. If the given element is not a tuple, nothing happens
	 */
	...
	/**
	 * replaces the given JValueTuple oldValue with the given newValue. 
	 * If the given element newValue is not a tuple, nothing happens
	 */
