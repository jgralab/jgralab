...
/**
 * JValueSet implements a mathematic set of JValue-Objects. This includes methods for union, 
 * difference, intersection etc.
 * It is based on the class <code>AbstractMathSet</code> and <code>HashMathSet</code> from the package
 * MathCollection which was developed in a project at the Institute for Intelligent Systems at the 
 * University of Stuttgart (http://www.iis.uni-stuttgart.de) under guidance of 
...
public class JValueSet extends JValueCollection  {
     ...
     * elements are returned in no particular order, this in includes that 
     * two calls of iterator() won't neccessary return the same order of elements. 
     *
    ...
    /**
     * Returns the hash code value for this jvalue-set. To get the hash
     * code of this set, new hash code values for every element of
    ...
    public boolean equals(Object o) {
        if (o instanceof JValueSet) {
            if (this.hashCode() == ((JValueSet)o).hashCode()) {
                return true;
            }
        }
        return false;
     ...
     * mathematical set or in the specified set. This set and the given one are unchanged
     *
	 ...
	 * the right type if you guess that a JValueCollection is a set
