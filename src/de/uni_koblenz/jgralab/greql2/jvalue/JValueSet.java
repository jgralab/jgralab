...
public class JValueSet extends JValueCollection {
	 ...
	 * mathematical set and the specified <code>HashMathSet</code> are
	 * compared. Since the hash code values are being cached, this represents a
	 * quick solution if the sets aren't equal. However, if the hash code values
	 * are equal, it cannot be assumed that the sets themselves are equal, since
	 ...
	 * <code>HashMathSet</code> containing all elements that are present in
	 * this mathematical set or in the specified set. This set and the given one
	 * are unchanged
	 * 
	 * @param s
	 ...
	 * specified set by invoking the <code>size()</code> method on each. If
	 ...
	 * is contained in the specified set. If it is so contained,
	 * <code>false</code> is returned. If the specified set has fewer
	 * elements, then the implementation iterates over the specified set,
