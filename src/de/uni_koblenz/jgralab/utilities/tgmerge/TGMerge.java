...
public class TGMerge {
	...
	private List<AbstractGraphMarker<AttributedElement>> additionalGraphMarkers = new LinkedList<AbstractGraphMarker<AttributedElement>>();
	...
	public TGMerge(Graph g, AbstractGraphMarker<AttributedElement>... markers) {
		if (markers.length == 0) {
			throw new RuntimeException("No marker given!");
		}
		targetGraph = g;
		for (AbstractGraphMarker<AttributedElement> m : markers) {
	...
	public Graph merge() {
		...
		for (AbstractGraphMarker<AttributedElement> marker : additionalGraphMarkers) {
			log.fine("Merging GraphMarker '" + marker + "'...");
			// Iterate over the complete graph instead of only the marked
			// elements to keep the order
			for (Vertex v : marker.getGraph().vertices()) {
				if (marker.isMarked(v)) {
					copyVertex(v);
				}
			}
			for (Edge e : marker.getGraph().edges()) {
