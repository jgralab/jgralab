...
public class OptimizerInfoGenerator {
	 ...
	 * 
	 * @param schema
	 *            a {@link Schema}, must not be <code>null</code>
	 */
	 ...
	 * 
	 * @param graph
	 *            a {@link Graph}, must have the same schema as this
	 *            OptimizerInfoGenerator
	 */
	...
	/**
	 * Scans all graphs in directory <code>dirName</code> whose names are
	 * accepted by the <code>filter</code>. When <code>recursive</code> is true,
	 * all subdirectories are included.
	 * 
	 * @param dirName
	 *            a directory name
	 * @param filter
	 *            a FilenameFilter for graph file names
	 * @param recursive
	...
	/**
	 * Scans all graphs in directory <code>dirName</code> whose names are
	 * accepted by {@link TGFilenameFilter}.
	 * 
	 * @param dirName
	 *            a directory name
	 * @param recursive
	...
	/**
	 * Scans all graphs in directory <code>dirName</code> whose names are
	 * accepted by {@link TGFilenameFilter}. When <code>recursive</code> is
	 * true, all subdirectories are included.
	 * 
	 * @param dirName
	 *            a directory name
	 * @throws GraphIOException
	 ...
	 * 
	 * @param propFilename
	 *            the name of the property file (should end with
	 *            ".optimizerinfo.properties")
	 * @throws IOException
	 *             when the file can not be stored
	 */
	...
	/**
	 * Computes the count of instances of <code>gec</code> in all scanned
	 * graphs.
	 * 
	 * @param gec
	 *            a {@link GraphElementClass}
	 * @param withSubclasses
	 *            when true, the result includes all subclasses of
	 *            <code>gec</code>
	 ...
	 *            when true, the result includes all subclasses of
	 *            <code>gec</code>
	 * @return the frequency <code>gec</code> in all scanned graphs, 0.0 <=
	 *         result <= 1.0
	 */
	...
	public void printStatistics(PrintStream out) {
		...
		for (GraphElementClass<?, ?> gec : schema.getGraphClass()
				.getGraphElementClasses()) {
			out.println((gec instanceof VertexClass ? "V" : "E") + "\t"
					+ gec.getQualifiedName() + "\t" + getCount(gec, false)
					+ "\t" + getCount(gec, true) + "\t"
					+ fmt.format(getFrequency(gec, false)) + "\t"
