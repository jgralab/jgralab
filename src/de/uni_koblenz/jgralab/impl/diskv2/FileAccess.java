 ...
 * provided by this class. The object returned by this method then provides
 * methods to write the contents of a ByteBuffer into this file, or to read a
 * number of bytes from this file, which are then returned in a byte buffer.
 * 
 * Internally, it uses a MappedByteBuffer for increased efficiency if the
 * operating system isn't Windows. If Windows is used, it sticks with a
 * FileChannel because a bug in Java makes it impossible for files to be deleted
 * if it has been accessed via a MappedByteBuffer at any point.
 * 
 * @author aheld
 * 
 */
public abstract class FileAccess {
	...
	/**
	 * Factory method that provides a FileAccess object for a specific file. If
	 * such an object was created previously, that object is returned. Else, a
	 * new object is created and returned.
	 * 
	 * @param filename
	 *            The name of the file to access. The suffix ".dst" is added
	 *            internally.
	...
	/**
	 * Writes the contents of a ByteBuffer into the file that this object
	 * provides access to.
	 * 
	 * @param content
	 *            The ByteBuffer whose content is written to the file
	 * @param index
	 *            The position in the file to which the content is written, in
	 *            bytes
	 */
	 ...
	 * @return A byte buffer containing the requested bytes
	 */
