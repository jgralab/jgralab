...
/**
 * This class is used to detect and store the attributes of generated vertex and
 * edge classes. It stores the type of every generated attribute and offers ways
 * to invoke the get- and set-methods for every attribute. For every
 * non-abstract vertex and edge class, there must be exactly one profile. A
 * profile is valid for all Edges and Vertices of the same class.
 * 
 * @author aheld
 * 
 */
public class GraphElementProfile {
	...
	/**
	 * The size that the profiled GraphElement needs on the disk. This includes
	 * variables, like kappa and firstIncidenceId, as well as all generated
	 * attributes that are of primitive types.
	 */
	private int size;

	/**
	 * The positions at which the addresses of the Strings and Lists will be
	 * stored on the disk, relative to the position where the data for the
	 * GraphElement begins. For example, if a vertex needs 80 bytes for its
	 * primitive attributes, then startOfStrings #will be 81.
	 */
	private int startOfStrings;
	private int startOfLists;

	/**
	 * Array holding the type IDs of the generated primitive attributes Each ID
	 * corresponds to a type. The table is: 0 -> boolean 1 -> integer 2 -> long
	 * 3 -> double
	 */
	private byte[] attrTypeIDs;

	/**
	 * Array holding the get method for every generated attribute that is a
	 * primitive
	 */
	private Method[] attrGetters;

	/**
	 * Array holding the set method for every generated attribute that is a
	 * primitive
	 * 
	 */
	...
	// - attrTypeIDs[n], getters[n] and setters[n] refer to the type, the get
	// method and the
	...
	/**
	 * Array holding the get method for every generated attribute that is a List
	 */
	private Method[] listGetters;

	/**
	 * Array holding the set method for every generated attribute that is a List
	 * 
	 */
	...
	public static int createProfile(VertexClass cls, int typeId, GraphFactory f) {
		GraphElementProfile profile = new GraphElementProfile(
				f.getVertexImplementationClass(cls),
	...
	/**
	 * Get a profile for the vertex or edge class which has the given internal
	 * ID.
	 * 
	 * @param typeId
	 *            The internal ID of the vertex or edge class
	 * @return A profile for the specified vertex or edge class
	 */
	 ...
	 * 
	 * @return How many bytes an object of the profiled class needs on the disk
	 */
	public int getSize() {
		return size;
	}
	...
	/**
	 * Get the number of attributes of the profiled vertex or edge class that
	 * are Strings
	 * 
	...
	/**
	 * Returns a ByteBuffer containing the primitive attributes of a
	 * GraphElement.
	 * 
	 * @param ge
	...
	public ByteBuffer getAttributesForElement(GraphElement<?, ?> ge) {
		...
		for (int i = 0; i < attrTypeIDs.length; i++) {
			...
			case 0: // attribute is a Boolean, invoke its get method and store
					// it
				...
				break;
			case 1: // attribute is an Integer, invoke its get method and store
					// it
			...
			case 2: // attribute is a Long, invoke its get method and store it
				...
				break;
			case 3: // attribute is an Double, invoke its get method and store
					// it
	...
	public void restoreAttributesOfElement(GraphElement<?, ?> ge, ByteBuffer buf) {
		for (int i = 0; i < attrTypeIDs.length; i++) {
			...
			// iterate over every attribute
			switch (attrTypeIDs[i]) {
			case 0: // attribute is a Boolean, invoke its set method and restore
					// it
				if (buf.get() == 1) {
					invokeSetBoolean(ge, true, i);
				} else {
					invokeSetBoolean(ge, false, i);
				}
				break;
			case 1: // attribute is an Integer, invoke its set method and
					// restore it
			...
			case 2: // attribute is a Long, invoke its set method and restore it
				...
				break;
			case 3: // attribute is an Double, invoke its set method and restore
					// it
	 ...
	 * 
	 * @return An Array containing the given GraphElement's Strings
	 */
	public String[] getStringsForElement(GraphElement<?, ?> ge) {
		if (stringGetters.length == 0) {
			return null;
		}
	 ...
	 * 
	 * @return An Array containing the given GraphElement's Lists
	 */
	public List<?>[] getListsForElement(GraphElement<?, ?> ge) {
		if (listGetters.length == 0) {
			return null;
		}
	 ...
	 * 
	 * @param cls
	 *            - The Edge or Vertex class to be profiled
	 */
	 ...
	 * 
	 * @param numAttrs
	 *            The length of the arrays attrTypeIDs, attrGetters and
	 *            attrSetters
	 * 
	 * @param numStrings
	 ...
	 * 
	 * @param primitive
	 *            A stack containing primitive data types, e.g. integer or
	 *            double.
	 */
	 ...
	 */
	private void detectGetters(Class<?> cls, Stack<Field> attributes,
			Method[] getters, byte[] attrTypeIDs) {
		String methodName;
		Method m;
		int index = 0;
	 ...
	 */
	private void detectSetters(Class<?> cls, Stack<Field> attributes,
			Method[] setters) {
		byte attrTypeID;
		String methodName;
		Method m = null;
	 ...
	 */
	private Method detectBooleanSetter(String methodName, Class<?> cls,
			Class<?>[] parameter) {
	 ...
	 */
	private Method detectIntegerSetter(String methodName, Class<?> cls,
			Class<?>[] parameter) {
	 ...
	 */
	private Method detectLongSetter(String methodName, Class<?> cls,
			Class<?>[] parameter) {
	 ...
	 */
	private Method detectDoubleSetter(String methodName, Class<?> cls,
			Class<?>[] parameter) {
	 ...
	 */
	private Method detectStringSetter(String methodName, Class<?> cls,
			Class<?>[] parameter) {
	 ...
	 */
	private Method detectListSetter(String methodName, Class<?> cls,
			Class<?>[] parameter) {
	 ...
	 * 
	 * @param ge
	 *            - The GraphElement whose set method shall be invoked. Its
	 *            class must be the same class that was passed to the
	 *            Constructor of the GraphElementProfile object for which this
	 *            method is called.
	 * @param argument
	 *            - The argument that is passed to the set method.
	 * @param position
	 *            - The position at which the set method invoked by this method
	 *            is stored in the array 'setters'. The type of this method's
	 *            argument must be the same as the type of the argument passed
	 *            to this method.
	 */
	private void invokeSetBoolean(GraphElement<?, ?> ge, boolean argument,
			int position) {
		try {
			attrSetters[position].invoke(ge, argument);
		} catch (IllegalAccessException e) {
			throw new IllegalArgumentException("Method "
					+ attrSetters[position].getName() + " is not public");
		} catch (InvocationTargetException e) {
			throw new IllegalArgumentException(
					"Exception thrown by invoked method "
							+ attrSetters[position].getName());
		}
	}
	 ...
	 *            to this method.
	 */
	private void invokeSetInteger(GraphElement<?, ?> ge, int argument,
			int position) {
		try {
			attrSetters[position].invoke(ge, argument);
		} catch (IllegalAccessException e) {
	 ...
	 *            to this method.
	 */
	private void invokeSetLong(GraphElement<?, ?> ge, long argument,
			int position) {
		try {
			attrSetters[position].invoke(ge, argument);
		} catch (IllegalAccessException e) {
	 ...
	 *            to this method.
	 */
	private void invokeSetDouble(GraphElement<?, ?> ge, double argument,
			int position) {
		try {
			attrSetters[position].invoke(ge, argument);
		} catch (IllegalAccessException e) {
	 ...
	 *            to this method.
	 */
	private void invokeSetString(GraphElement<?, ?> ge, String argument,
			int position) {
		try {
	 ...
	 *            to this method.
	 */
	private void invokeSetList(GraphElement<?, ?> ge, List<?> argument,
			int position) {
		try {
	 ...
	 * 
	 * @param ge
	 *            - The GraphElement whose get method shall be invoked. Its
	 *            class must be the same class that was passed to the
	 *            Constructor of the GraphElementProfile object for which this
	 *            method is called.
	 * @param position
	 *            - The position at which the get method invoked by this method
	 *            is stored in the array 'getters'. The invoked method's return
	 *            type must be the same as the return type of this method.
	 */
	private boolean invokeGetBoolean(GraphElement<?, ?> ge, int position) {
		try {
			...
			throw new IllegalArgumentException("Method "
					+ attrGetters[position].getName() + " is not public");
		} catch (InvocationTargetException e) {
			throw new IllegalArgumentException(
					"Exception thrown by invoked method "
							+ attrGetters[position].getName());
		}
	}
	...
	/**
	 * Helper method to return the name of the type for a given type ID. Used to
