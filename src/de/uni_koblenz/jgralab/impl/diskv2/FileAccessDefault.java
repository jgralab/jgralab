...
/**
 * Implementation for the abstract class FileAccess that is used if the
...
public class FileAccessDefault extends FileAccess {
	 ...
	 * 
	 * @param filename
	 *            The name of the file the constructed object provides access
	 *            to.
	 */
	protected FileAccessDefault(String filename) {
		...
		try {
			...
			// use invalid values to force a remapping at the first read or
			// write operation
			firstByte = lastByte = -1;
		} catch (FileNotFoundException e) {
			throw new RuntimeException("Error: Could not create file "
					+ filename + ".dst");
		}
	}
	...
	/**
	 * Checks if the current read or write operation fits into the access
	 * window, i.e. if the part of the file that the operation wants to access
	 * is currently in the memory.
	 * 
	 * If it is, this method does nothing. If it isn't, the changes made to the
	 * part of the file that was mapped to the memory when this method was
	 * called are forced out to the disk. After that, a new part of the file is
	 * mapped into memory. The area that is mapped into memory is chosen in such
	 * a way that the start position of the read or write operation (i.e. the
	 * byte at which the read or write operation is started) is in the middle of
	 * the area that is mapped to the memory during this method call. An
	 * exception to this rule occurs if the condition "index < FILE_AREA/2" is
	 * met. In this case, the first FILE_AREA bytes of the file are mapped to
	 * the memory.
	 * 
	 * @param bufSize
	 *            The amount of bytes that are read or written in the next
	 *            operation
	 * @param index
	...
	private void checkAccessWindow(int bufSize, long index) {
		if (index < firstByte | index + bufSize > lastByte) {
			...
			try {
				...
				accessWindow = channel.map(MapMode.READ_WRITE, firstByte,
						FILE_AREA);
			} catch (IOException e) {
