...
public abstract class ReversedAggregationImpl extends ReversedEdgeImpl implements Aggregation {

	public ReversedAggregationImpl(EdgeImpl normalEdge, Graph graph) {
		super(normalEdge, graph);
	}
	
	public Aggregation getNextAggregationInGraph() {
		return ((Aggregation)normalEdge).getNextAggregationInGraph();
	}

	public Aggregation getNextAggregationInGraph(boolean noSubClasses) {
		return ((Aggregation)normalEdge).getNextAggregationInGraph(noSubClasses);
	}
