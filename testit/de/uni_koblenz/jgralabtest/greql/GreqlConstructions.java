...
public class GreqlConstructions extends GenericTest {
	...
	/*
	 * Test method for
	 * 'greql2.evaluator.GreqlEvaluator.evaluateForwardVertexSet(
	 * ForwardVertexSet, Graph)'
	 */
	@Test
	public void testPathSystemConstruction() throws Exception {
		String queryString = "from c: V{localities.County} " + "with c.name = 'Rheinland-Pfalz' "
				+ "report pathSystem(c, -->{localities.ContainsLocality} -->{connections.AirRoute}) " + "end";
		Object result = evalTestQuery("PathSystemConstruction", queryString, TestVersion.ROUTE_MAP_GRAPH);
